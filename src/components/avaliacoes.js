export function renderAvaliacoesSection(avaliacoes, mediaNotas, totalAvaliacoes, currentUserId) {
    ...
    <div class="bg-white rounded-2xl shadow-lg p-6 mb-8">
      ...
      <div class="flex items-center gap-4 mb-6 p-4 bg-gray-50 rounded-xl">
        <div class="text-center">
          <p class="text-4xl font-bold text-gray-800">${media}</p>
          <div class="flex items-center justify-center mt-1">
            ${renderStars(parseFloat(media))}
          </div>
          <p class="text-xs text-gray-500 mt-1">${totalAvaliacoes} ${totalAvaliacoes === 1 ? 'avaliação' : 'avaliações'}</p>
        </div>
      </div>
      ...
      ` : `
        <p class="text-gray-500 text-center py-6">Ainda não existem avaliações. Sê o primeiro a avaliar!</p>
      `}
    </div>

    ${renderAvaliacaoModal()}
  `;
}
...
function renderAvaliacaoCard(av, currentUserId) {
  ...
  return `
    <div class="border border-gray-100 rounded-xl p-4 hover:bg-gray-50 transition-all">
      <div class="flex items-start justify-between mb-2">
        <div>
          <div class="flex items-center gap-2 mb-1">
            <span class="font-semibold text-gray-800">${av.autor_nome || 'Utilizador'}</span>
        ...
        <div class="flex items-center gap-2">
          ...
          ${isAuthor ? `
            <button class="deleteAvaliacaoBtn text-gray-400 hover:text-red-500 transition-colors" data-id="${av.id}" title="Eliminar avaliação">
      ...
      ${av.comentario ? `<p class="text-gray-600 text-sm mt-2 leading-relaxed">${escapeHtml(av.comentario)}</p>` : ''}
    </div>
  `;
}
...
function renderStars(nota) {
  ...
  for (let i = 1; i <= 5; i++) {
    if (i <= Math.floor(nota)) {
      ...
      stars += '<svg class="h-4 w-4 text-yellow-400" fill="currentColor" viewBox="0 0 24 24"><path d="M12 2l3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-6.18 3.25L7 14.14 2 9.27l6.91-1.01L12 2z" opacity="0.5"></path><path d="M12 5.5l1.76 3.56 3.94.57-2.85 2.78.67 3.93L12 14.27l-3.52 1.85.67-3.93-2.85-2.78 3.94-.57L12 5.5z" fill="currentColor"></path></svg>';
    } else {
      ...
      <div class="bg-white rounded-2xl shadow-2xl max-w-md w-full p-8">
        ...
        <form id="avaliacaoForm" class="space-y-5">
          ...
          <div>
            ...
            <p id="notaLabel" class="text-sm text-gray-500 mt-1">Selecione uma nota</p>
          </div>
          ...
          <div>
            ...
            <textarea
              ...
              placeholder="Escreve o teu comentário (opcional)"
            ></textarea>
          </div>
